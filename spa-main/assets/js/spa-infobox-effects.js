/**
 * SPA Infobox Wizard – Effects (DOM / AJAX / Events)
 * VŠETKY SIDE EFFECTS A ASYNCHRÓNNE OPERÁCIE
 */
...
window.watchFormChanges = function() {
    ...
    document.addEventListener('change', function(e) {
        ...
        if (e.target.name === spaConfig.fields.spa_city) {
            // ⭐ KRITICKÝ RESET PROGRAMU (PRED INOU LOGIKOU)
...
window.applyGetParams = function() {
    ...
    setTimeout(() => {
        ...
        const checkOptions = setInterval(() => {
			...
			console.log('[SPA GET DEBUG] Matched option text:', matchedOption.text);                    setTimeout(() => {
                    ...
                    }, 500);                    window.wizardData.city_name = matchedOption.text.trim();
                    ...
                    console.log('[SPA GET] ✅ City applied:', matchedOption.text);                    citySelect.dispatchEvent(new Event('change', { bubbles: true }));                    let verifyCityAttempts = 0;
                    const maxVerifyCityAttempts = 10;                    const verifyCityValue = setInterval(() => {
                        verifyCityAttempts++;
                        const currentCitySelect = document.querySelector(`[name="${spaConfig.fields.spa_city}"]`);
                        const currentValue = currentCitySelect?.value;                        if (currentValue && currentValue === matchedOption.value) {
            ...
            setTimeout(() => {
                ...
                const maxProgramAttempts = 10;                const checkProgramOptions = setInterval(() => {
                    programAttempts++;
                    const programSelect = document.querySelector(`[name="${spaConfig.fields.spa_program}"]`);
                    const hasProgramOptions = programSelect && programSelect.options.length > 1;                    console.log('[SPA GET] Waiting for program options... (' + programAttempts + '/' + maxProgramAttempts + ')');                    if (!hasProgramOptions && programAttempts < maxProgramAttempts) {
                        return;
                    }                    clearInterval(checkProgramOptions);                    if (!hasProgramOptions) {
		    ...
		    const matchedOption = Array.from(programSelect.options).find(opt => 
				    ...
				    const freshOption = Array.from(programSelect.options).find(opt => opt.value == programParam);
                                    if (freshOption) {
                                        programSelect.value = freshOption.value;                                        if (typeof jQuery !== 'undefined' && jQuery(programSelect).data('chosen')) {
                    ...
                    }, 3000);                    const programBackup = document.getElementById('spa_program_backup');
                    if (programBackup) {
                    ...
                    window.spaGFGetState.programApplied = true;                    programSelect.dispatchEvent(new Event('change', { bubbles: true }));                    console.log('[SPA GET] ✅ Program applied:', matchedOption.text);                    let verifyAttempts = 0;
                    const maxVerifyAttempts = 20;                    const verifyProgramValue = setInterval(() => {
                        verifyAttempts++;
                        const currentSelect = document.querySelector(`[name="${spaConfig.fields.spa_program}"]`);
                        const currentValue = currentSelect?.value;                        if (currentValue && currentValue == programParam) {
                    ...
                    }, 100);                    window.loadInfoboxContent(window.currentState);
...
/**
 * SPA Infobox – INIT
 * Spúšťa side-effects po načítaní DOM
 */
(function spaInfoboxInit() {
    console.log('[SPA INIT] Infobox effects init');
    ...
    function init() {
        if (!window.spaConfig || !spaConfig.fields) {
            console.error('[SPA INIT] spaConfig not ready');
            return;
        }

        console.log('[SPA INIT] Binding form logic');

        // 1️⃣ Naviazanie event listenerov
        if (typeof window.watchFormChanges === 'function') {
            window.watchFormChanges();
        } else {
            console.error('[SPA INIT] watchFormChanges missing');
        }

        // 2️⃣ Aplikácia GET parametrov
        if (typeof window.applyGetParams === 'function') {
            window.applyGetParams();
        }

        // 3️⃣ Obnova wizardData z hidden polí
        if (typeof window.restoreWizardData === 'function') {
            window.restoreWizardData();
        }

        // 4️⃣ Initial state sync
        if (typeof window.updateInfoboxState === 'function') {
            window.updateInfoboxState();
        }

        console.log('[SPA INIT] DONE');
    }
})();
