...
/* ==========================
   AJAX: Načítanie programov podľa mesta
   ========================== */
   ...
   /**
    * AJAX endpoint: Vráti zoznam programov pre dané mesto
    * 
    * POST parametre:
    * - city_name: názov mesta (slug alebo text)
    * 
    * Response:
    * - success: true/false
    * - data: array of {id, title}
    */
   function spa_ajax_get_programs() {
       // Bezpečnosť: sanitizácia vstupu
       $city_name = isset($_POST['city_name']) ? sanitize_text_field($_POST['city_name']) : '';
       
       if (empty($city_name)) {
           wp_send_json_error(['message' => 'Mesto nie je zadané']);
           return;
       }
       
       error_log('[SPA AJAX] Loading programs for city: ' . $city_name);
       
       // WP_Query: Programy filtrované podľa mesta (taxonómia spa_place)
       $args = [
           'post_type' => 'spa_group',
           'posts_per_page' => -1,
           'post_status' => 'publish',
           ...
           'tax_query' => [[
               'taxonomy' => 'spa_place',
               'field' => 'slug',
               'terms' => sanitize_title($city_name)
           ]]
       ];
       ...
       if (!$query->have_posts()) {
           error_log('[SPA AJAX] No programs found for: ' . $city_name);
           wp_send_json_success([]);
           return;
       }
       
       // Konverzia na array {id, title}
       $programs = [];
       while ($query->have_posts()) {
           ...
           $programs[] = [
               'id' => get_the_ID(),
               'title' => get_the_title(),
               'data_program_id' => get_the_ID() // Pre kompatibilitu s existujúcim JS
           ];
       }
   ...
   /**
    * Inicializácia AJAX (volané z spa_registration_init)
    */
